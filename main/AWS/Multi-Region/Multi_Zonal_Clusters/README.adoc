= Multi-Region Deployment: Multi-Zonal (HA) Configuration

This configuration consists of a cluster of nodes that are deployed within an AWS data center in the ```us-east-1``` region, connected to two other clusters of nodes, deployed respectively within the ```eu-west-3``` and ```ap-northeast-1``` regions. Each node is deployed in a distinct availability zone.
...
image::images/Multi-Region.svg[Multi-Region Multi-Zonal Configuration]

As per above architecture, we create :

- 3 Regions - US-East-1, EU-West-2, and AP-NorthEast-1,
